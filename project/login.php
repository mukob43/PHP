<?php
    session_start(); //start the session to use session
    ...
    $error = "";
     if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        //เอาค่าที่รับจากฟอร์มไปตรวจสอบว่ามีข้อมูลตรงใน db หรือไม่
        $sql = "SELECT * FROM users WHERE (username = ? OR email = ?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$usernameOrEmail,$email]); 
        ...
        if($user && password_verify($password,$user['password'])){
            ...
            exit(); //หยุดการทำงานของสคริปดักหลังจากเปลี่ยนเส้นทาง
        }else{
